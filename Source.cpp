...
{
	ifstream Input;
	Input.open("C:\\Users\\marye\\Desktop\\Uber\\Input.txt");
	int TotalTowns;
...
AdjList::~AdjList()
{
	//Saving to file
	ofstream Output;
	Output.open("C:\\Users\\marye\\Desktop\\Uber\\Input.txt");
...
customer::customer()
{
	...
	ifstream myfile;
	myfile.open("C:\\Users\\marye\\Desktop\\Uber\\customer2.txt");
	if (myfile.is_open()) {
		int s;
		myfile >> s;
		myfile.ignore();
...
void MapOfDrivers::Get_Data() {
	...
	driver_file.open("C:\\Users\\marye\\Desktop\\Uber\\Driver.txt");
	if (driver_file.fail())
		cout << "Fail" << endl;
	Driver d;
	int s;
